...
def analyze_pdf_document(file_path):
    ...
    try:
        # 1. 파일 업로드 (가장 최신 라이브러리 규격 적용)
        # 만약 여기서 에러가 난다면 라이브러리 버전 문제일 수 있습니다.
        with open(file_path, "rb") as f:
            uploaded_doc = client.files.upload(file=f)
        ...
        # 에러 발생 시 화면에 구체적인 원인 출력 (디버깅용)
        st.error(f"⚠️ 분석 엔진 오류: {str(e)}")
        return {
            "title": "기술 분석 지연",
            "summary": ["현재 AI 모델 응답이 원활하지 않습니다.", "SMK 원본 파일을 참고해 주세요.", f"에러내용: {str(e)[:50]}"],
            "category": "기타"
        }
    finally:
