...
def analyze_pdf_document(file_path):
    ...
    try:
        ...
        # 2. 파일 업로드 시 mime_type을 명시적으로 설정하여 오류 방지
        # 'application/pdf'를 직접 지정합니다.
        ...
        # 에러 발생 시 구체적인 원인 출력
        st.error(f"⚠️ 분석 엔진 오류: {str(e)}")
        return {
            "title": "기술 분석 지연",
            "summary": ["PDF 인식 오류가 발생했습니다.", "잠시 후 다시 시도해 주세요.", f"상세에러: {str(e)[:50]}"],
            "category": "기타"
        }
    finally:
        # 로컬 임시 파일 삭제 (NameError 방지를 위해 안전하게 처리)
        if 'temp_process_path' in locals() and os.path.exists(temp_process_path):
